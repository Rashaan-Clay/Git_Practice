...
#define EXIT_INVALID_POINTER 91
#define EXIT_READ_ONLY_POINTER 90
...
do { \
    ...
    /**
     * @brief Macro for calling function that returns a pointer in a forked process, without any validation of the pointer
     */
#define CALL_FUNCTION_POINTER_NO_VALIDATION(function_name, args...) \
do { \
    TEST_ASSERT_FALSE_MESSAGE(not_implemented_##function_name, "The " #function_name " function has not been implemented."); \
    pid_t child_pid = fork(); \
    if (child_pid < 0) \
...
/**
 * @brief Macro for calling function that returns a pointer in a forked process, validating that the pointer is NULL or valid
 */
...
do { \
    ...
    { \
        ...
        if (!is_pointer_null_or_valid(resultPtr)) \
        { \
            exit(EXIT_INVALID_POINTER); \
        } \
        \
        void* testPtr = malloc(ctest_get_malloc_size(resultPtr)); \
        if (testPtr == NULL) \
...
/**
 * @brief Macro for calling function that returns a pointer that is not to be NULL in a forked process, ensuring that it is valid
 */
...
do { \
 ...
 /**
  * @brief Macro for calling function that returns a writeable string in a forked process
  */
#define CALL_FUNCTION_WRITEABLE_STRING(function_name, args...) \
do { \
    TEST_ASSERT_FALSE_MESSAGE(not_implemented_##function_name, "The " #function_name " function has not been implemented."); \
    pid_t child_pid = fork(); \
    if (child_pid < 0) \
    ...
    { \
        ...
        \
        if (is_pointer_read_only(resultPtr)) \
        { \
            exit(EXIT_READ_ONLY_POINTER); \
        } \
        \
        size_t result_size = strlen(resultPtr); \
        memset(testPtr, 0, strlen(resultPtr) + 1); \
...
/**
 * @brief Macro for calling function that returns a string in a forked process, returning the value
 */
...
do { \
 ...
 /**
  * @brief Macro for calling function that returns a writeable string in a forked process, returning the value
  */
#define CALL_FUNCTION_WRITEABLE_STRING_WITH_RETURN(function_name, string_size, args...) \
char* return_value_##function_name = NULL; \
do { \
    TEST_ASSERT_FALSE_MESSAGE(not_implemented_##function_name, "The " #function_name " function has not been implemented."); \
    key_t shm_key = (key_t) rand(); \
...
else if (WEXITSTATUS(child_return) == EXIT_INVALID_POINTER) \
{ \
    TEST_FAIL_MESSAGE("An invalid pointer was returned when testing the " #function_name " function."); \
} \
else if (WEXITSTATUS(child_return) == EXIT_READ_ONLY_POINTER) \
{ \
    TEST_FAIL_MESSAGE("An read-only pointer was unexpectedly returned when testing the " #function_name " function."); \
} \
...
{ \
    ...
    { \
        error_message = "An invalid pointer was returned when testing the " #function_name " function."; \
	    test_failed = true; \
    } \
    else if (WEXITSTATUS(child_return) == EXIT_READ_ONLY_POINTER) \
    { \
        error_message = "An read-only pointer was unexpectedly returned when testing the " #function_name " function."; \
	    test_failed = true; \
    } \
    else if (child_return != 0) \
    { \
...
/**
 * @brief Macro to ensure a pointer is valid, displaying a message if it is not
 */
#define TEST_ASSERT_POINTER_VALID_MESSAGE(pointer, message) \
do { \
    TEST_ASSERT_TRUE_MESSAGE(is_pointer_valid(pointer), message); \
} while(0)

 /**
  * @brief Macro to ensure a pointer is valid
  */
#define TEST_ASSERT_POINTER_VALID(pointer) \
do { \
    TEST_ASSERT_TRUE_MESSAGE(is_pointer_valid(pointer), "The " #pointer " pointer was not valid."); \
} while(0)

  /**
   * @brief Macro to ensure a pointer is NULL or valid, displaying a message if it is not
   */
#define TEST_ASSERT_POINTER_NULL_OR_VALID_MESSAGE(pointer, message) \
do { \
    TEST_ASSERT_TRUE_MESSAGE(is_pointer_null_or_valid(pointer), message); \
} while(0)

   /**
    * @brief Macro to ensure a pointer is NULL or valid
    */
#define TEST_ASSERT_POINTER_NULL_OR_VALID(pointer) \
do { \
    TEST_ASSERT_TRUE_MESSAGE(is_pointer_null_or_valid(pointer), "The " #pointer " pointer was either not NULL or was not valid."); \
} while(0)
...
/**
 * @brief Builds a full path for a file that should be in the current directory
 * @param filename Filename to use
 * @return Full path
 */
...
/**
 * @brief Determines if the captured output contains the specified regex pattern
 * @param pattern Pattern to test
 * @return True if the file contains the pattern, false otherwise
 */
 ...
 * @return Captured output or NULL if there was none
 */
...
/**
 * @brief Signal handler function for use with segmentation faults
 * @param sig Signal raised
 */
void sigsegv_handler(int sig);

/**
 * @brief Determines if a pointer is valid
 * @param ptr Pointer to test
 * @return True if pointer is not NULL and is valid; false otherwise
 */
bool is_pointer_valid(const void* ptr);

/**
 * @brief Determines if a pointer is NULL or valid
 * @param ptr Pointer to test
 * @return True if pointer is NULL or is valid; false otherwise
 */
bool is_pointer_null_or_valid(const void* ptr);

/**
 * @brief Determines if pointer is read-only
 * @param ptr Pointer to test
 * @return True/false if pointer is read-only
 */
 ...
 * @return True if string is in a valid non-NULL memory location and printable, false if not
 */
bool is_printable(const char* string);

/**
 * Implementation of nanosleep function that restarts if interrupted by a signal
 *
 * @param seconds Number of seconds to sleep
 * @param nanoseconds Number of nanoseconds to sleep
 * @return 0 on success, or -1 if an error other than EINTR occurred.
 */
int robust_nanosleep(int seconds, long nanoseconds);
